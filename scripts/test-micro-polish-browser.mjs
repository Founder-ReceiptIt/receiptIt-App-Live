...
for(const [name,width,height,mobile] of [['Small',320,568,true],['Android',360,640,true],['Pixel',393,873,true],['iPhone',390,844,true],['Samsung',412,915,true],['Desktop',1280,800,false]]){
 ...
 await page.getByText('Warranty',{exact:true}).evaluate(el=>el.closest('section').scrollIntoView({block:'center',behavior:'instant'}));await page.waitForTimeout(500);await fit(page,name+' Detail');
 ...
 for(const [merchant,w,r] of [['Warranty only',true,false],['Returns only',false,true],['Neither',false,false],['Urgent return',false,true],['Expired cover',true,true]]){
  ...
  assert.equal(await page.getByText('Warranty',{exact:true}).count(),w?1:0,merchant+' warranty');
  assert.equal(await page.getByText('Returns',{exact:true}).count(),r?1:0,merchant+' returns');await fit(page,name+' '+merchant);
  if(phase!=='before'&&merchant==='Expired cover'){
   const shadows=await page.getByText('Warranty',{exact:true}).evaluate(el=>Array.from(el.closest('section').children).map(card=>getComputedStyle(card).boxShadow));
   assert.deepEqual(shadows,['none','none'],'Expired cover must stay muted without coloured glow');
  }
  if(name==='Pixel'){
   const label=page.getByText(w?'Warranty':'Returns',{exact:true});
