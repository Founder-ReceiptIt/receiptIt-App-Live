        ...
        <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-6 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {isSignUp && !isForgotPassword ? <div>
              ...
              <p className={`mt-2 flex min-h-5 items-center gap-1.5 text-xs ${aliasState === 'available' ? 'text-teal-300' : aliasState === 'unavailable' ? 'text-amber-200' : 'text-gray-500'}`}>{aliasState === 'available' ? <Check className="h-3.5 w-3.5" /> : null}{aliasMessage || 'Use this when a retailer only needs an email to send your receipt.'}</p>
            </div> : null}
