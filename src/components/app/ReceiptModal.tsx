...
export function ReceiptModal({ receipt, onClose, onDelete }: ReceiptModalProps) {
  ...
  const handleProofPack = async () => {
    ...
    try {
      ...
      showToast('Proof pack ready', 'Opened in a new tab.');
    } catch (error) {
        ...
        >
          <div className="bg-[#0b0f0f]/95 border border-white/10 rounded-3xl overflow-hidden shadow-[0_26px_80px_rgba(0,0,0,.52)]">
            ...
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <h2 id="receipt-title" className="text-2xl font-bold text-white">Receipt</h2>
              <div className="flex items-center gap-2">
                {hasOriginalReceipt && isFinalizedReceiptStatus(receipt.status) && (
                  ...
                  >
                    ...
                    <span>{isGeneratingProofPack ? 'Preparing…' : 'Create proof pack'}</span>
                  </motion.button>
                )}
                {hasOriginalReceipt && (
                  <motion.button
