...
export function InsightsTab() {
  ...
  if (loading) return <div className="mx-auto max-w-6xl px-6 pb-32 pt-8"><div className="h-8 w-32 animate-pulse rounded bg-white/10" /><div className="mt-8 grid gap-4 sm:grid-cols-3"><div className="h-32 animate-pulse rounded-2xl bg-white/[0.045]" /><div className="h-32 animate-pulse rounded-2xl bg-white/[0.045]" /><div className="h-32 animate-pulse rounded-2xl bg-white/[0.045]" /></div></div>;
  if (error) return <div className="mx-auto max-w-2xl px-6 pb-32 pt-8"><h1 className="text-3xl font-bold text-white">Insights</h1><div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.045] p-8 text-center"><p className="text-gray-300">Could not load insights right now.</p><button onClick={() => setRefreshKey((value) => value + 1)} className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-teal-300"><RefreshCw className="h-4 w-4" />Try again</button></div></div>;
    ...
    <div className="mx-auto max-w-6xl px-6 pb-32 pt-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
        ...
        {receipts.length === 0 ? <div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.045] p-10 text-center"><BarChart3 className="mx-auto h-8 w-8 text-teal-300" /><p className="mt-4 text-gray-300">More insights will appear as you add receipts.</p></div> : <>
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-2"><section className="rounded-2xl border border-white/10 bg-white/[0.045] p-5 sm:p-6"><h2 className="flex items-center gap-2 text-lg font-bold text-white"><Tag className="h-4 w-4 text-teal-300" />By category</h2><div className="mt-5 space-y-4">{summary.byCategory.map(([category, amount]) => <div key={category}><div className="flex justify-between gap-4 text-sm"><span className="text-gray-300">{category}</span><span className="font-semibold text-white">{formatMoney(amount)}</span></div><div className="mt-2 h-1.5 rounded-full bg-white/10"><div className="h-full rounded-full bg-teal-400/70" style={{ width: `${(amount / maxCategory) * 100}%` }} /></div></div>)}</div></section><section className="rounded-2xl border border-white/10 bg-white/[0.045] p-5 sm:p-6"><h2 className="flex items-center gap-2 text-lg font-bold text-white"><Store className="h-4 w-4 text-teal-300" />Top merchants</h2><div className="mt-4 divide-y divide-white/10">{summary.byMerchant.map(([merchant, amount]) => <div key={merchant} className="flex items-center justify-between gap-4 py-3 text-sm"><span className="truncate text-gray-300">{merchant}</span><span className="shrink-0 font-semibold text-white">{formatMoney(amount)}</span></div>)}</div></section></div>
        </>}
      </motion.div>
    </div>
  );
