...
export function InsightsTab() {
  ...
  const saveBudget = () => {
    ...
    if (!Number.isInteger(budget / 10)) {
      setBudgetError('Use whole £10 amounts for your monthly budget.');
      return;
    }
...
function BudgetEditor({
  ...
  return <section className="mt-6 max-w-3xl rounded-2xl border border-white/10 bg-white/[0.045] p-5 sm:p-6"><div className="flex flex-col gap-1"><h2 className="text-lg font-bold text-white">Monthly budget</h2><p className="text-sm text-gray-400">Set a monthly budget to see your progress in Wallet.</p></div><div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end"><label className="flex-1 text-xs font-semibold uppercase tracking-[0.14em] text-gray-500">Budget amount<span className="mt-2 flex items-center gap-2 rounded-xl border border-white/10 bg-black/20 px-3"><span className="text-base text-gray-400">£</span><input aria-label="Monthly budget" type="number" min="10" step="10" inputMode="numeric" value={budgetInput} onChange={(event) => onInputChange(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') onSave(); }} className="w-full bg-transparent py-2.5 text-base font-semibold text-white outline-none placeholder:text-gray-600" placeholder="2500" /></span></label><div className="flex gap-2"><button type="button" onClick={onSave} className="rounded-xl bg-teal-400 px-4 py-3 text-sm font-bold text-black transition-colors hover:bg-teal-300">Save budget</button>{monthlyBudget ? <button type="button" onClick={onClear} className="rounded-xl border border-white/15 px-4 py-3 text-sm font-semibold text-gray-300 transition-colors hover:bg-white/5">Use default</button> : null}</div></div>{budgetError ? <p className="mt-3 text-sm text-amber-200">{budgetError}</p> : null}</section>;
}
