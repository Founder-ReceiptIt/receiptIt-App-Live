      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }}>
        ...
        <div className="mx-auto max-w-2xl text-center">
          <section className="mt-8 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.045] p-6 sm:p-8">
            ...
            <p className="mt-6 text-2xl font-bold text-white sm:text-3xl">{aliasLocalPart && aliasDomain ? <><span className="whitespace-nowrap">{aliasLocalPart}@</span><wbr /><span className="whitespace-nowrap">{aliasDomain}</span></> : emailAlias || 'No address set'}</p>
