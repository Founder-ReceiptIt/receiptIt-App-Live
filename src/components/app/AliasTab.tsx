      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }}>
        ...
        <div className="mx-auto mt-5 max-w-3xl">
          <section className="overflow-hidden rounded-2xl border border-teal-300/25 bg-teal-400/[0.055] p-5 text-center shadow-[0_0_32px_rgba(45,212,191,0.07)] sm:p-7">
            ...
            <p className="mt-5 min-w-0 break-all text-xl font-bold text-white min-[360px]:text-2xl sm:text-3xl">{aliasLocalPart && aliasDomain ? <><span className="inline-block max-w-full break-all">{aliasLocalPart}@</span><wbr /><span className="inline-block max-w-full break-all">{aliasDomain}</span></> : emailAlias || 'No email set'}</p>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><Mail className="h-4 w-4 text-teal-300" /><h2 className="mt-3 text-xs font-bold uppercase tracking-[0.12em] text-teal-100">Stop the spam</h2><p className="mt-2 text-sm leading-6 text-gray-400">Give retailers your receiptIt email instead of your personal inbox when they only need somewhere to send the receipt.</p></section>
            <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><Send className="h-4 w-4 text-teal-300" /><h2 className="mt-3 text-xs font-bold uppercase tracking-[0.12em] text-teal-100">Send receipts here</h2><p className="mt-2 text-sm leading-6 text-gray-400">Forward receipts, invoices or order confirmations to your receiptIt address and we’ll add them for you.</p></section>
          </div>
        </div>
      </motion.div>
    </div>
