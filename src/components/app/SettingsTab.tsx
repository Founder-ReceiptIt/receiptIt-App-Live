...
import { AlertTriangle, Download, FileText, ShieldCheck, Trash2, X } from 'lucide-react';
...
export function SettingsTab() {
  ...
  const handleExport = async () => {
    ...
    const headers = ['Status', 'Document type', 'Date', 'Merchant', 'Amount', 'Currency', 'Category', 'Reference'];
    ...
    <div className="mx-auto max-w-2xl px-6 pb-32 pt-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
        ...
        <section className="mt-8 rounded-2xl border border-white/10 bg-white/[0.045] p-6">
          <div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-teal-400/25 bg-teal-400/10"><ShieldCheck className="h-5 w-5 text-teal-300" strokeWidth={1.7} /></div><div><h2 className="text-lg font-bold text-white">Privacy &amp; security</h2><p className="mt-1 text-sm leading-6 text-gray-400">Your receipts and purchase data are private to your account.</p><button type="button" onClick={() => setShowProtectionDetail((value) => !value)} aria-expanded={showProtectionDetail} className="mt-3 text-sm font-semibold text-teal-300 transition-colors hover:text-teal-200">How receiptIt protects your data</button>{showProtectionDetail && <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.16 }} className="mt-3 max-w-xl text-sm leading-6 text-gray-400">Your account can access your saved documents and the purchase information we extract from them. You can download or permanently delete your data whenever you need to.</motion.p>}</div></div>
        </section>
        ...
        <button onClick={() => void signOut()} className="mt-6 min-h-11 text-sm font-semibold text-gray-400 transition-colors hover:text-white">Sign out</button>
