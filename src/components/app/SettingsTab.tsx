  ...
  return (
    <div className="ri-mobile-page mx-auto min-w-0 max-w-7xl px-4 pt-8 sm:px-6">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
        ...
        {showDeleteModal && <div className="ri-scroll-viewport bg-black/60 backdrop-blur-sm" onClick={() => !deleteLoading && setShowDeleteModal(false)}><div className="ri-scroll-viewport__inner"><motion.div initial={{ opacity: 0, y: 8, scale: 0.985 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.2 }} className="w-full min-w-0 max-w-sm rounded-2xl border border-white/15 bg-neutral-950 p-5 shadow-2xl sm:p-6" onClick={(event) => event.stopPropagation()}><div className="flex items-start gap-3"><div className="rounded-xl border border-red-400/25 bg-red-400/10 p-2"><AlertTriangle className="h-5 w-5 text-red-300" /></div><div className="min-w-0 flex-1"><h2 className="text-lg font-bold text-white">Delete account?</h2><p className="mt-2 text-sm leading-6 text-gray-400">This permanently deletes your account and everything you’ve saved. This cannot be undone.</p></div><button aria-label="Close" onClick={() => setShowDeleteModal(false)} disabled={deleteLoading} className="shrink-0 text-gray-500 transition-colors hover:text-white"><X className="h-5 w-5" /></button></div><div className="mt-6 flex gap-3"><button onClick={() => setShowDeleteModal(false)} disabled={deleteLoading} className="flex-1 rounded-lg border border-white/10 px-3 py-2.5 text-sm font-semibold text-gray-200 hover:bg-white/5">Cancel</button><button onClick={() => void handleDeleteAccount()} disabled={deleteLoading} className="flex-1 rounded-lg border border-red-300/25 bg-red-400/15 px-3 py-2.5 text-sm font-semibold text-red-100 hover:bg-red-400/25 disabled:opacity-50">{deleteLoading ? 'Deleting…' : 'Delete account'}</button></div></motion.div></div></div>}
      </motion.div>
    </div>
  );
}
